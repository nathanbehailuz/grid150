-- Let owners choose the group's active/rest-day schedule at creation time.
...
create or replace function public.create_group(
  ...
  p_active_days smallint[] default '{1,2,3,4,5}'
)
returns table (
  group_id uuid,
  invite_code text
...
declare
  ...
  v_active smallint[] := p_active_days;
begin
  ...
  if v_active is null
    or cardinality(v_active) = 0
    or not (v_active <@ array[0,1,2,3,4,5,6]::smallint[])
    or cardinality(v_active) <> (
      select count(distinct day)::integer from unnest(v_active) as day
    )
  then
    raise exception 'active days must contain unique weekdays from 0 to 6';
  end if;
  ...
  set default_daily_new_target = v_daily,
